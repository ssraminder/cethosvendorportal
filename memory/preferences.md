...
- **Anon uploads to `cvp-applicant-cvs` MUST use a UUID as the first path segment.** The `cvp_applicant_cvs_anon_insert` RLS policy enforces `(storage.foldername(name))[1] ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'`. Any other prefix (e.g. `agency-profiles/{uuid}/...`) returns 403 with `row-level security policy` and the browser's `supabase.storage.upload()` silently fails — no console error, the form just sits there.
  - **Why:** PR #228 used `agency-profiles/{uuid}/...` for the agency company profile and would have 403'd every real submission. PR #229 hotfixed to `{uuid}/profile_{filename}`. Caught only because we ran a live API-level submission to verify.
  - **How to apply:** When adding any new anon-uploadable file type to the recruitment app, use the path shape `{crypto.randomUUID()}/{descriptive_prefix_}{sanitized_filename}` — never a custom top-level folder. Carry the file's purpose on the application row (e.g. `applicant_type='agency'` vs `'individual'`, or a `*_path` column name), not in the storage path.
